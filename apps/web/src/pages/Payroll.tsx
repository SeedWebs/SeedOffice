...
function SelfView() {
  ...
  if (!d) return <div className="p-6 text-sm text-slate-400">กำลังโหลด…</div>
    ...
    <div className="p-3 sm:p-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-w-4xl items-start">
        ...
        <div className="bg-white rounded-lg shadow-xs p-6">
          <div className="text-sm text-slate-400">สรุปเวลาของฉัน — {cycleLabel(d.cycle)}</div>
          <div className="flex items-start gap-3 mt-3">
            <div>
              <div className="text-3xl font-bold tabular-nums text-slate-900">{minutesToHoursLabel(d.minutesTotal)}</div>
              <div className="text-[11px] text-slate-400">ชั่วโมงงวดนี้</div>
            </div>
            <span className={`ml-auto text-[11px] px-2 py-1 rounded-full ${d.flagged ? 'bg-orange-100 text-orange-700' : 'bg-emerald-50 text-emerald-600'}`}>
              manual {Math.round(d.manualRatio * 100)}% · {d.flagged ? 'สูงกว่าปกติ' : 'ปกติ'}
            </span>
          </div>
          <div className="text-[11px] text-slate-400 mt-1.5">วันนี้ {minutesToHoursLabel(d.todayMinutes)} ชม. · เป้า 8 ชม./วัน</div>
          <div className="mt-4 space-y-2">
            <div className="text-[11px] font-medium text-slate-400 uppercase tracking-wide">ตามโปรเจกต์</div>
            {d.byProject.length === 0 && <div className="text-sm text-slate-300">ยังไม่มีเวลาในงวดนี้ — กดจับเวลาจากงานได้เลย</div>}
            {d.byProject.map((p) => (
              <div key={p.projectId} className="flex items-center gap-2">
                <span className="text-sm text-slate-600 flex-1 truncate">{p.projectName}</span>
                <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-brand-400" style={{ width: `${(p.minutes / maxProjectMinutes) * 100}%` }} />
                </div>
                <span className="text-xs text-slate-500 tabular-nums w-16 text-right">{minutesToHoursLabel(p.minutes)} ชม.</span>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-4">
          {d.ownerNote && (
            <div className="bg-amber-50 border border-amber-100 rounded-lg p-4">
              ...
              <p className="text-sm text-slate-700 mt-1.5">{d.ownerNote}</p>
            </div>
          )}
          <div className="bg-white rounded-lg shadow-xs p-6">
            <div className="text-sm text-slate-400">{isVendor ? 'ค่าจ้างของฉัน' : 'ค่าตอบแทนของฉัน'} — {cycleLabel(d.cycle)}</div>
            <div className="mt-4 space-y-2">
              <div className="text-[11px] font-medium text-emerald-700 uppercase tracking-wide">รายได้</div>
                ...
                return (
                  <div key={kind} className="flex items-center justify-between">
                    <span className="text-sm text-slate-500 flex items-center gap-1">
                      {KIND_LABEL[kind]} {kind === 'bonus' && <Lock className="w-3 h-3 text-slate-300" />}
                    </span>
                    <span className={`tabular-nums ${amt === 0 ? 'text-slate-300' : kind === 'bonus' ? 'text-amber-700' : ''}`}>
                      {amt === 0 ? '—' : formatSatang(amt)}
                    </span>
                  </div>
                )
              })}
              <div className="text-[11px] font-medium text-rose-600 uppercase tracking-wide pt-2">หัก</div>
              {DEDUCT_ORDER.filter((k) => !isVendor || k === 'wht' || sumByKind(k) > 0).map((kind) => {
                const amt = sumByKind(kind)
                return (
                  <div key={kind} className="flex items-center justify-between">
                    <span className="text-sm text-slate-500">{KIND_LABEL[kind]}</span>
                    <span className={`tabular-nums ${amt === 0 ? 'text-slate-300' : 'text-rose-500'}`}>
                      {amt === 0 ? '—' : `-${formatSatang(amt)}`}
                    </span>
                  </div>
                )
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                <span className="text-sm font-medium text-slate-700">รวมสุทธิ</span>
                <span className="text-xl font-bold tabular-nums text-slate-900">{formatSatang(d.netSatang)}</span>
              </div>
            </div>
