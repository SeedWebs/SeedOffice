...
function PnlChip({ projectId }: { projectId: string }) {
  ...
  const barColor = p.health === 'red' ? 'bg-danger-500' : p.health === 'amber' ? 'bg-warning-400' : 'bg-brand-500'
  return (
    <div className="group relative flex items-center gap-2 text-xs">
      <span className="text-muted">progress</span>
      <div className="w-28 h-2 bg-divider rounded-full overflow-hidden">
        <div className={`h-full ${barColor}`} style={{ width: `${Math.min(100, pct ?? p.usagePct ?? 0)}%` }} />
      </div>
      <span className={`font-medium ${p.health === 'red' ? 'text-danger-600' : p.health === 'amber' ? 'text-warning-600' : 'text-dim'}`}>
        {pct != null ? `${pct}%` : p.usagePct != null ? `${p.usagePct}%` : '—'}
      </span>
      <div className="absolute z-10 right-0 top-full mt-2 w-56 bg-ink text-white rounded-xl p-3 opacity-0 group-hover:opacity-100 pointer-events-none transition shadow-lg">
        <div className="flex justify-between py-0.5">
          <span className="text-border">ชั่วโมง</span>
          <span>{minutesToHoursLabel(p.minutesTotal)}{p.estimateMinutes > 0 ? ` / ${minutesToHoursLabel(p.estimateMinutes)}` : ''}</span>
        </div>
        <div className="flex justify-between py-0.5">
          <span className="text-border">ต้นทุน{p.quotedSatang != null ? ' / ขาย' : ''}</span>
          <span>{formatSatang(p.costSatang)}{p.quotedSatang != null ? ` / ${formatSatang(p.quotedSatang)}` : ''}</span>
        </div>
        {p.profitSatang != null && (
          <div className="flex justify-between py-0.5 border-t border-body mt-1 pt-1">
            <span className="text-border">กำไร</span>
            <span className={p.profitSatang >= 0 ? 'text-brand-300' : 'text-danger-300'}>
...
function GroupTimeline({ groups }: { groups: BoardGroup[] }) {
  ...
  const COLORS = ['bg-warning-400', 'bg-brand-500', 'bg-info-500', 'bg-violet-500', 'bg-orange-400', 'bg-danger-500', 'bg-success-500']

  return (
    <div className="bg-white rounded-lg shadow-xs p-4 mb-4">
      <div className="flex items-baseline gap-2 mb-3">
        <span className="text-sm font-semibold text-body">ไทม์ไลน์</span>
        <span className="text-[11px] text-muted">ช่วง = งานที่เริ่มเร็วสุด → จบช้าสุด ในกลุ่ม</span>
      </div>
          ...
          {ranged.map((r, i) => (
            <div key={r.name} className="flex items-center py-1">
              <div className="w-44 shrink-0 text-sm pr-3 truncate">
                <span className="font-medium text-body">{r.name}</span>
              </div>
    ...
    <div className="p-3 sm:p-6">
        ...
        {canEdit && (
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 text-xs border rounded-lg px-2.5 py-1.5 ${reorderOn ? 'bg-brand-50 border-brand-200 text-brand-700' : 'text-dim border-border-subtle hover:bg-hover'}`}
            >
      ...
      <div className="space-y-4">
        {groups.map((g, gi) => (
          ...
          >
            <div
              className="flex items-center gap-2 px-4 py-3 border-b border-border-subtle bg-hover/50"
              draggable={reorderOn}
              onDragStart={() => setDrag({ kind: 'group', id: g.id })}
            >
              {reorderOn && <GripVertical className="w-4 h-4 text-border cursor-grab" />}
              <span className="font-semibold text-strong text-sm">{g.name}</span>
              <span className="text-xs text-muted">{g.tasks.length} งาน</span>
              {canEdit && (
                <button onClick={() => { setAddingTaskIn(addingTaskIn === g.id ? null : g.id); setNewTitle('') }} className="ml-auto text-xs text-muted hover:text-brand-600 flex items-center gap-1">
            ...
            <div className="divide-y divide-hover">
              {g.tasks.map((t, ti) => (
                <div key={t.id}>
                  ...
                  >
                    {reorderOn && <GripVertical className="w-4 h-4 text-border-subtle cursor-grab shrink-0" />}
                    <button
                      ...
                      className={`w-4 h-4 rounded-full border-2 grid place-items-center shrink-0 ${
                        t.status === 'done' ? 'border-brand-400 bg-brand-400 text-white' : t.status === 'doing' ? 'border-warning-400' : 'border-border'
                      }`}
                    >
                      {t.status === 'done' && <Check className="w-2.5 h-2.5" />}
                    </button>
                    <span className={`text-sm flex-1 min-w-0 truncate ${t.status === 'done' ? 'text-muted line-through' : ''}`}>{t.title}</span>
                    <button
                      ...
                      className="shrink-0"
                    >
                      <Star className={`w-4 h-4 ${t.starredToday ? 'text-warning-400 fill-warning-400' : 'text-border-subtle hover:text-warning-300'}`} />
                    </button>
              ...
              {g.tasks.length === 0 && !reorderOn && (
                <div className="px-4 py-4 text-sm text-border">ยังไม่มีงานในกลุ่มนี้</div>
              )}
              {addingTaskIn === g.id && (
                <div className="flex items-center gap-2 px-4 py-2.5 bg-hover/60">
                  <input
                    autoFocus
                    value={newTitle}
                  ...
                  <button onClick={() => setAddingTaskIn(null)} className="text-muted"><X className="w-4 h-4" /></button>
                </div>
              )}
            </div>
          </div>
        ...
        {canEdit && (
          addingGroup ? (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setAddingGroup(false)} className="text-muted"><X className="w-5 h-5" /></button>
            </div>
          ) : (
            <button onClick={() => setAddingGroup(true)} className="w-full text-sm text-muted hover:text-brand-600 border-2 border-dashed border-border-subtle hover:border-brand-300 rounded-lg py-3 flex items-center justify-center gap-1">
