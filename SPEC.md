...
- [ ] **PAT ราย user** (`api_tokens`): สุ่ม `sko_<hex>` เก็บเป็น **SHA-256 hash** เหมือน session (token หลุดจาก DB ใช้ไม่ได้) · โชว์ token เต็ม **ครั้งเดียว**ตอนสร้าง · ฟิลด์: name, scopes, lastUsedAt, revokedAt (soft-revoke)
- [ ] **สร้าง/ลิสต์/เพิกถอน** ที่ **หน้าโปรไฟล์ (Profile) → Access Tokens** (ตั้งค่า = owner-only จึงย้ายมาโปรไฟล์ที่ owner+member เข้าได้) — **owner + member เท่านั้น** (vendor ❌ ใน v1) · แต่ละคนจัดการ token ของตัวเอง · owner เพิกถอนของใครก็ได้
- [ ] **Auth**: middleware รับ `Authorization: Bearer sko_…` เป็นทางเลือกของ session cookie · token → resolve user (ต้อง active) + แนบ scopes · audit เดินเหมือน user คนนั้นทำเอง (actor = เจ้าของ token, meta ระบุ via-token + token name)
- [ ] **Scope v1 = งาน + เวลาเท่านั้น**: `tasks:read` `tasks:write` `time:read` `time:write` (+ `projects:read` ไว้อ่านชื่อโปรเจกต์/กลุ่มตอน assign) — **ไม่มี scope การเงิน**
- [ ] **การเงินปิดสนิทต่อ PAT**: route payroll/finance/expenses/clients/crm = **cookie session เท่านั้น** (PAT ไม่มี cookie → 401 อัตโนมัติ) · permission test ยืนยัน PAT แตะ payroll/bonus/P&L = **401/403 เสมอ**
- [ ] **สิทธิ์ซ้อน (defense in depth)**: PAT ทำได้ ≤ (role ของ user) ∩ (scopes ของ token) — role check เดิมในแต่ละ handler ยังทำงาน (เช่น vendor อนาคต ลงเวลาได้เฉพาะของตัวเอง)
- [x] **อ่าน — `GET /api/me/today`** (PAT `tasks:read`): งานติดดาว/รับผิดชอบวันนี้ + เวลารวมวันนี้/เมื่อวาน (รูปเดียวกับ standup §4.6) สำหรับ `/checkin`
...
- [ ] **สร้าง task ใหม่จาก PAT = ยังไม่อยู่ใน v1** (`POST /api/groups/:id/tasks` ต้องรู้ groupId ซึ่ง discover ได้แค่ผ่าน board cookie-only) — PAT ทำงานกับ task ที่มีอยู่ (assign/status/star/ลงเวลา); สร้างใหม่ทำผ่าน UI
- [x] ทุก write ผ่าน PAT → `audit_logs` (เดินเหมือน user คนนั้น) · Zod ทุก endpoint
- [ ] **MCP server** หุ้ม REST (tools: list_my_tasks / create_task / assign_task / log_time / today) — **เฟสสุดท้าย** หลัง REST+PAT ใช้จริงแล้ว
> เฟส: **A** ฐาน PAT (table+middleware+ตั้งค่า) → **B** scope-gate endpoint + `/api/me/today` → **C** ต่อ `/checkin` (ฝั่ง wiki/`.claude`) → **D** MCP
