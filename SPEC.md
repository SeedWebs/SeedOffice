...
- [ ] sync ขาเข้า: v1 = **Cron ทุก 1 นาที** polling (History API), later = Pub/Sub push → webhook (near-realtime) · เชื่อมกล่องเสร็จ = **backfill ~50 ฉบับล่าสุด** ทันที · historyId เก่าเกิน (404) → fallback ดึงตามช่วงเวลา · refresh token ใช้ไม่ได้ → กล่องเป็น "หลุดการเชื่อมต่อ" + โชว์ error ใน ตั้งค่า · ข้าม TRASH/DRAFT, SPAM → สถานะ spam
