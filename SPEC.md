...
> สถานะ: **Draft v0.7** — spec coherent + sync กับ mockup · **แผน P1 (tasks/plan.md + todo.md) refresh แล้ว — พร้อมเริ่ม build**
...
> **IA:** เมนูหลัก = **ภาพรวม · โปรเจกต์ · อีเมลกลาง · ค่าตอบแทน · เงินสดย่อย · ตั้งค่า** — งาน/เวลา อยู่ใน "โปรเจกต์" ทั้งหมด (ไม่มีหน้า Tasks/ลงเวลา/Standup แยก; standup อยู่บน ภาพรวม)
...
| `projects` | id, code, name, logo, clientName, type(project\|ma), status, quotedSatang (int), billingType(fixed\|recurring), recurringPeriod(monthly\|yearly)?, startDate, dueDate | 2 ประเภท |
...
│  │  ├─ src/routes/       # หน้า: overview, projects (+ project detail), inbox, payroll, expenses, admin
│  │  ├─ src/components/
│  │  ├─ src/lib/          # api client, auth context, formatters (เงิน/เวลา/timezone)
│  │  └─ src/hooks/
│  └─ api/                 # Hono worker
│     ├─ src/routes/       # /auth /projects /tasks /time /payroll /expenses /calendar /inbox /admin
...
- **E2E (Playwright)** — flow หลัก: login → ลงเวลา → เห็นเงินตัวเองอัปเดต; owner ปิด payroll; member ลง petty cash
...
- **P2:** Standup (auto) + Petty cash + **Dashboard team hub** (team activity/presence + ปฏิทินทีม + Quick Add) — presence ต้องมี realtime (DO+WS)
...
- IA/header ใหม่: ภาพรวม·โปรเจกต์·อีเมลกลาง·ค่าตอบแทน·เงินสดย่อย·ตั้งค่า; standup บน ภาพรวม; header = h1+action, role switcher แยก top bar
...
- **Backup** ข้อมูลการเงิน (D1 export ตามรอบ)
