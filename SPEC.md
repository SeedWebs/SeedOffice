...
- [ ] **Endpoint**: reuse `POST/PATCH /api/tasks` + `POST/PATCH /api/time` (เพิ่ม scope-gate) + เพิ่ม **`GET /api/me/today`** = งานติดดาว/รับผิดชอบวันนี้ + เวลารวมวันนี้/เมื่อวาน (รูปเดียวกับ standup §4.6) สำหรับ `/checkin`
- [ ] ทุก write ผ่าน PAT → `audit_logs` · Zod ทุก endpoint
